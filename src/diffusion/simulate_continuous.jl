...
function simulate(
    h::Hypergraph, 
    type::Type{ContinuousNonLinearConsensus};
    max_iter::Union{Int,Float64}=3.0,
    step_size::Union{Int,Float64}=0.01,
    ...
    while t < max_iter
        ...
        for v in 1:nhv(h)
            ...
            verbose && println("v: $v - curr_op: $(curr_opinions[v]) - gain: $gain - next_op: $(curr_opinions[v] + Δ * gain)")
            # verbose && println("gains: $gains")
            # verbose && println("curr_opinions: $curr_opinions")
            verbose && println("------------------------------------\n")

            # NB this is a difference with the discrete case
            next_op_v = curr_opinions[v] + Δ * gain
        ...
        if abs(std(next_opinions .- curr_opinions)) < 0.0005
            println("convergence reached at $t/$max_iter iteration")
            break
        end
